...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	for name, query := range uq.withNamedReviews {
		if err := uq.loadReviews(ctx, query, nodes,
			func(n *User) { n.appendNamedReviews(name) },
			func(n *User, e *Review) { n.appendNamedReviews(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (uq *UserQuery) WithNamedReviews(name string, opts ...func(*ReviewQuery)) *UserQuery {
	query := (&ReviewClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if uq.withNamedReviews == nil {
		uq.withNamedReviews = make(map[string]*ReviewQuery)
	}
	uq.withNamedReviews[name] = query
	return uq
}
